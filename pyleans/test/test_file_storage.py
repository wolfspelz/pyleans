...
class TestFileStorageRead:
    async def test_read_nonexistent_returns_empty(self, storage: FileStorageProvider) -> None:
...
class TestFileStorageWrite:
    ...
    async def test_write_with_correct_etag(self, storage: FileStorageProvider) -> None:
    ...
    async def test_write_with_wrong_etag_raises(self, storage: FileStorageProvider) -> None:
    ...
    async def test_write_with_none_etag_overwrites(self, storage: FileStorageProvider) -> None:
        await storage.write("Counter", "c1", {"value": 1}, None)
        await storage.write("Counter", "c1", {"value": 2}, None)
...
class TestFileStorageClear:
    ...
    async def test_clear_nonexistent_is_noop(self, storage: FileStorageProvider) -> None:
        await storage.clear("Counter", "nope", None)

    async def test_clear_with_wrong_etag_raises(self, storage: FileStorageProvider) -> None:
        await storage.write("Counter", "c1", {"value": 1}, None)
        with pytest.raises(StorageInconsistencyError):
            await storage.clear("Counter", "c1", "wrong-etag")

    async def test_clear_with_none_etag(self, storage: FileStorageProvider) -> None:
        await storage.write("Counter", "c1", {"value": 1}, None)
...
class TestFileStorageRoundTrip:
    async def test_full_lifecycle(self, storage: FileStorageProvider) -> None:
        ...
        etag2 = await storage.write("Player", "p1", {"name": "Alice", "level": 6}, etag)
...
class TestFileStoragePathTraversal:
    ...
    async def test_grain_type_traversal_blocked(self, storage: FileStorageProvider) -> None:
        path = storage._grain_path("../../../etc", "passwd")
        assert ".." not in str(path.relative_to(storage._base_path))

    async def test_grain_key_traversal_blocked(self, storage: FileStorageProvider) -> None:
        path = storage._grain_path("Counter", "../../../etc/passwd")
        assert ".." not in str(path.relative_to(storage._base_path))

    async def test_slashes_in_grain_type_sanitized(self, storage: FileStorageProvider) -> None:
        path = storage._grain_path("a/b/c", "key")
        assert path.parent.name != "c"

    async def test_backslashes_in_type_sanitized(self, storage: FileStorageProvider) -> None:
        path = storage._grain_path("a\\b\\c", "key")
        assert "\\" not in path.parent.name

    async def test_resolved_path_within_base(self, storage: FileStorageProvider) -> None:
        path = storage._grain_path("Counter", "key")
        assert str(path).startswith(str(storage._base_path))

    async def test_empty_grain_type_raises(self, storage: FileStorageProvider) -> None:
        with pytest.raises(StorageError, match="empty after sanitization"):
            storage._grain_path("", "key")

    async def test_empty_grain_key_raises(self, storage: FileStorageProvider) -> None:
        with pytest.raises(StorageError, match="empty after sanitization"):
...
class TestFileStorageCorruptionHandling:
    ...
    async def test_corrupt_json_raises_storage_error(self, storage: FileStorageProvider) -> None:
        await storage.write("Counter", "c1", {"value": 1}, None)
        path = storage._grain_path("Counter", "c1")
