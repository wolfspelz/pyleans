# Task 32: Local Directory Cache With Invalidation
...
Without a cache, every grain call on a non-owner silo incurs a directory RPC round-trip. Orleans' answer is a per-silo cache of recent lookups, invalidated on membership change. Any cache hit means the cost of "where does this grain live?" drops from milliseconds to microseconds; cache misses fall back through to [task-31](task-31-distributed-grain-directory.md).
...
1. **Membership change** -- the `MembershipAgent` ([task-29](task-29-failure-detector.md)) calls `cache.invalidate_all()` whenever the active-silo set changes. Cheaper than recomputing which entries moved; membership changes are rare.
...
4. **Per-call failure** -- the runtime in [task-34](task-34-remote-grain-invoke.md) catches `TransportConnectionError` from a grain call, calls `cache.invalidate(grain_id)`, and retries exactly once before failing the caller.
