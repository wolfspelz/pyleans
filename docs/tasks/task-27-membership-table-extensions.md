# Task 27: Membership Table Extensions -- Versioning, ETags, Suspicion, IAmAlive
...
- [task-12-yaml-membership.md](task-12-yaml-membership.md) (Phase 1 baseline provider to extend)
...
Phase 1 models membership as "write row, read rows, heartbeat timestamp" -- enough for a single silo to report itself, not enough for distributed agreement. Phase 2 needs the table to serve as the **rendezvous point** for the failure detector ([task-29](task-29-failure-detector.md)): it must durably record suspicion votes and status transitions with optimistic concurrency so multiple silos writing concurrently reach a consistent outcome.
...
- `src/pyleans/pyleans/identity.py` -- extend `SiloInfo` (already seeded with optional fields in [task-19](task-19-cluster-identity.md), now fully populated)
...
1. Acquire file lock (implemented in [task-28](task-28-file-locking-membership.md)).
2. Load file.
...
[Task-28](task-28-file-locking-membership.md) is a pure cross-cutting concern. This task describes the algorithm assuming the locking primitive is available; implementing it wrong is risky enough to deserve its own review boundary.
