# Task 26: `TcpClusterTransport` -- Concrete `IClusterTransport` Adapter
...
- [task-25-silo-connection-manager.md](task-25-silo-connection-manager.md)
...
Wire the pieces together: `TcpClusterTransport` implements `IClusterTransport` (from [task-22](task-22-transport-abcs.md)) by composing the `SiloConnectionManager` (task-25), `SiloConnection` (task-24), wire codec (task-23), plus an `asyncio.start_server` listener that hands accepted sockets to the manager.
...
The `Silo` (extended in [task-35](task-35-silo-lifecycle-stages.md)) owns the transport and drives its `start()` / `stop()` at the right lifecycle stages. This task:

- Does not mutate `Silo` -- that wiring is task-35.
- Exposes a **standalone integration test** that constructs two transports in the same process, wires them through loopback sockets, and exercises `send_request` both directions. This proves the whole stack works end-to-end before task-35 integrates it into the silo.

### Local-silo short-circuit

A `send_request` where `target == local_silo` is a programming bug in the caller (the runtime should resolve "is local" before dispatching through the transport). Detect and raise `ValueError` rather than loop back through the network -- silent self-communication hides routing bugs. The runtime in task-34 is responsible for routing local calls directly to `GrainRuntime.invoke()` without touching the transport.
