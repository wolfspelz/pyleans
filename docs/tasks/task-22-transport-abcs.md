# Task 22: Transport Ports -- `IClusterTransport` ABC, Message Types, Error Hierarchy
...
Concretely, this task ships **only ABCs, value types, and the error hierarchy**. The concrete TCP implementation is wired across [task-23](task-23-wire-protocol.md), [task-24](task-24-silo-connection.md), [task-25](task-25-silo-connection-manager.md), and [task-26](task-26-tcp-cluster-transport.md). Landing the port first freezes the contract the downstream work targets.
...
class TransportOptions:
    max_message_size: int = 16 * 1024 * 1024       # 16 MB -- reject larger, see task-23
...
`on_connection_established` / `on_connection_lost` are the integration point for the failure detector ([task-29](task-29-failure-detector.md)) and the directory ([task-32 cache invalidation](task-32-directory-cache.md)). Making them part of the port contract -- rather than letting each consumer poll `get_connected_silos()` -- keeps those consumers event-driven and lets us test membership-driven behaviors in isolation by driving callbacks directly.

### Handler return contract

`MessageHandler` returns `TransportMessage | None`:
- For `MessageType.REQUEST` input, the handler **must** return a `TransportMessage` with `message_type=RESPONSE` and the matching `correlation_id`. If the application raises, the adapter wraps it into a `MessageType.ERROR` response (error taxonomy in task-23).
...
- [ ] `MessageType` values match the wire codepoints in [architecture/pyleans-transport.md §4.3](../architecture/pyleans-transport.md) so task-23 can reuse the enum directly
