# Task 19: Cluster Identity -- `ClusterId`, Silo Id Format, Grain Id Hashing
...
2. A **canonical silo id string** (`host:port:epoch`) used in logs, membership rows, transport handshakes, and the hash ring. The string form must be stable and comparison-safe (lexicographic order is used for connection-dedup tie-breaking in [task-25](task-25-silo-connection-manager.md)).
...
class ClusterId:
    ...
    reach each other's network endpoints (see task-23 wire protocol).
    """
    value: str
...
- `version: int = 0` -- monotonic version attached by the membership table at write time (full use in [task-27](task-27-membership-table-extensions.md))
