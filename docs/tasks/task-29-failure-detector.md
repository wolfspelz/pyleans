# Task 29: Failure Detector -- Probe Protocol, Suspicion, Voting, Self-Monitoring
...
Probes ride the same connection used for grain calls -- a probe that succeeds when grain calls are failing is a bug in the transport layer ([task-24](task-24-silo-connection.md)), not the detector.
...
One per silo, started during `BecomeActive` lifecycle stage ([task-35](task-35-silo-lifecycle-stages.md)):
