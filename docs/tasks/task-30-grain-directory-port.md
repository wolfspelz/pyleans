# Task 30: Grain Directory Port -- `IGrainDirectory` ABC and Local Adapter
...
1. [Task-31](task-31-distributed-grain-directory.md) can then drop in a `DistributedGrainDirectory` implementation without touching runtime call paths.
...
class IGrainDirectory(ABC):
    ...
    async def resolve_or_activate(
        ...
        especially careful about (see task-31)."""
```
...
if entry.silo != self._local_silo:
    return await self._remote_invoke(entry, ...)     # stub — wired up in task-34
if grain_id not in self._activations:
    await self.activate_grain(grain_id)
...
```

In Phase 2 dev mode, `entry.silo` is always the local silo and the remote branch is dead code. That is fine -- it is the seam that task-34 wires into `TcpClusterTransport.send_request`. Landing this seam now means task-34 can be reviewed as a small delta rather than a runtime rewrite.
