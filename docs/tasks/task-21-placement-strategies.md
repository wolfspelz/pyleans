# Task 21: Grain Placement Strategies -- Random and PreferLocal
...
When a grain has no activation anywhere in the cluster, the grain directory (task-31) must pick a silo to host it. Orleans exposes this as a `PlacementStrategy` abstraction and ships several implementations; pyleans PoC ships two: **random** (baseline) and **prefer-local** (keeps chattiness local when the caller is already on a valid silo).
...
The `placement` decorator attaches the strategy class to the grain class attribute `_placement_strategy` (instantiated lazily). The directory (task-31) calls `grain_class._placement_strategy.pick_silo(...)` when resolving a missing activation. Grains without an explicit decorator default to `RandomPlacement`.
...
The directory (task-31) catches this and surfaces it to the caller as a `ClusterNotReadyError` -- distinguishing "no silos" (configuration / startup) from "all silos unreachable" (network) helps operations.
